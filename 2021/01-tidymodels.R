...
modelo <- especificacao_modelo %>%
  fit(price ~ x + y + z, data = diamonds)

print(modelo)

# tidymodels r um conjunto de pacotes
# https://parsnip.tidymodels.org/articles/articles/Models.html

diamonds_com_previsão <-
  diamonds %>%
  add_column(predict(modelo, new_data = diamonds)) # previsão
diamonds_com_previsão
